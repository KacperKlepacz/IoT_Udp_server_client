...
{ 
	...
	if(strcmp(argv[1],"IPv4") == 0){
		...
		for(;;){
            ...
            printf("%s\n",timestamp);
			float r3 = 24.f + ((rand()%100)/100.f);
			int x = sprintf(buffer,"%s\n%s: %.2f ºC",timestamp,argv[3],r3);
...
void help(char *arg[]){
	...
	printf("IoT temperature client program usage:");
	printf("%s <IPV4|IPV6> <server_ip_addr> <name_of_temperature_sensor>\n",arg[0]);
	printf("e.g.:%s IPV4 192.168.1.14 Temperature_sensor_1\n",arg[0]);
